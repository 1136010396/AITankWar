...
class EnemyManager:
    def __init__(self, game):
        ...
        self.role_configs = {
            ...
            C.TankRole.SUPPORTER: {
                "shoot_frequency": 0.1,  # 中等射击频率
                "move_speed": 4,  # 较快的移动速度
                "route_preference": [C.AttackRoute.LEFT, C.AttackRoute.RIGHT]  # 偏好侧路
            },
            C.TankRole.DEFENDER: {
                "shoot_frequency": 0.1,  # 较低的射击频率
                "move_speed": 2,  # 较慢的移动速度
                "route_preference": [C.AttackRoute.LEFT, C.AttackRoute.RIGHT]  # 偏好侧路
            }
        }

    def is_spawn_point_available(self, point):
        """检查生成点是否可用"""
        # 转换为像素坐标
        spawn_x = point["x"] * 24 + 3
        spawn_y = point["y"] * 24 + 3

        # 创建坦克大小的碰撞矩形
        spawn_rect = pygame.Rect(spawn_x, spawn_y, 48, 48)
        ...
        # 检查是否与玩家坦克碰撞
        if spawn_rect.colliderect(self.game.player.rect):
            return False

        # 检查是否与地形碰撞
        for terrain_group in self.game.map_manager.terrain_groups.values():
            for terrain in terrain_group:
                if spawn_rect.colliderect(terrain.rect):
                    return False

        return True

    def get_available_spawn_point(self, route):
        """获取指定路线的可用生成点"""
        # 获取该路线的所有生成点
        route_spawn_points = self.tactical_points[route][:2]

        # 检查每个生成点
        for point in route_spawn_points:
            if self.is_spawn_point_available(point):
                return point

        return None

    def create_enemy(self, role, route, spawn_point):
        """创建敌方坦克"""
        # 转换为像素坐标
        x = spawn_point["x"] * 24 + C.MAP_OFFSET
    ...
    def update_enemy_behavior(self, enemy):
        ...
        if enemy.current_path and enemy.current_path_index < len(enemy.current_path):
            ...
            if enemy.route == C.AttackRoute.MIDDLE:
                print()

            # 检查是否已经在目标位置
            if (abs(enemy.rect.x - next_pixel_x) == 0 and
                    abs(enemy.rect.y - next_pixel_y) == 0):
    ...
    def find_alternative_route(self, preferred_routes):
        """寻找替代路线"""
        # 检查所有可能的路线
        all_routes = list(C.AttackRoute)
        random.shuffle(all_routes)  # 随机打乱顺序

        # 优先检查首选路线
        for route in preferred_routes:
            if self.get_available_spawn_point(route):
                return route

        # 如果首选路线都不可用，检查其他路线
        for route in all_routes:
            if route not in preferred_routes and self.get_available_spawn_point(route):
                return route
    ...
    def spawn_enemy(self):
        ...
        if C.TankRole.ATTACKER not in current_roles:
            ...
            preferred_routes = self.role_configs[role]["route_preference"]
            # route = random.choice(self.role_configs[role]["route_preference"])

        # 寻找可用的路线
        route = self.find_alternative_route(preferred_routes)
        if not route:
            return

        # 获取可用的生成点
        spawn_point = self.get_available_spawn_point(route)
        if not spawn_point:
            return  # 以防万一

        # 创建坦克
        enemy = self.create_enemy(role, route,spawn_point)
