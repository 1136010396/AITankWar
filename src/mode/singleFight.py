...
from src.control.enemy_managerEasy import EnemyManager
from src.data.maps.map_data import MAPS

# 闯关模式
class SingleFightGame:
    def __init__(self,mapSelect):
        self.minStep=3
        self.maxStep=7
        self.map=MAPS[mapSelect]
        self.width = len(self.map) * C.LITTLE_BLOCK[0] + C.MAP_OFFSET * 2
        self.height = len(self.map[0]) * C.LITTLE_BLOCK[0] + C.MAP_OFFSET * 2
    ...
    def init(self):
        ...
        # 先暂时写死测试地图
        self.map_manager.load_map(self.map)
    ...
    def draw(self):
        ...
        score_text = font.render(f"Score: {self.score}/10", True, C.RED)
        self.screen.blit(score_text, (10, 10))

        # if self.game_over:
        #     font = pygame.font.Font(None, 72)
        #     game_over_text = font.render("Game Over", True, C.RED)
        #     self.screen.blit(game_over_text, (C.SCREEN_WIDTH // 2 - 100, C.SCREEN_HEIGHT // 2 - 36))
...
if __name__ == "__main__":
    list=["ORIGINAL_MAP","RIVER_MAP","EMPTY_MAP"]
    game = SingleFightGame(list[0])
    # 如果要切换到其他地图
